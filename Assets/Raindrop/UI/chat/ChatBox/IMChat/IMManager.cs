...
namespace Raindrop
{
    //this class handles all the logic for incoming and outgoing IM.
    //also handle the printing of IM....
    public class IMManager : MonoBehaviour
    {
        ...
        public IMPresenter presenter { get; set; }
        ...
        private void PrintIM(DateTime timestamp, string fromName, UUID fromID, string message, bool isNewMessage)
        {
            if (ShowTimestamps)
            {
                if (fontSettings.ContainsKey("Timestamp"))
                {
                    ...
                    //TextPrinter.BackColor = fontSetting.BackColor;
                    //TextPrinter.Font = fontSetting.Font;
                    TextPrinter.PrintText(DateTime.Now.ToString("[HH:mm] "), Color.gray);
                }
                else
                {
                    //TextPrinter.ForeColor = SystemColors.GrayText;
            ...
            {
                TextPrinter.PrintText(fromName, Color.black);
            }
        ...
        public void ProcessChatInput(string inputString, ChatType normal/*, UUID target, UUID SessionId*/)
        {
            if (inputString.Length == 0) return;

            textBuffer.Add(inputString);
            // textPointer = textBuffer.Count;

            // string msg = inputString.Replace(ChatInputBox.NewlineMarker, "\n");
